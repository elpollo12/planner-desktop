...
export default function ReportView() {
  ...
  const handleExport = async (format: 'pdf' | 'excel' | 'both') => {
    ...
    try {
      ...
      if (result.saved) toast.success(`${format === 'both' ? 'Reportes guardados' : format === 'pdf' ? 'PDF guardado' : 'Excel guardado'} exitosamente`);
    } catch (error) {
    ...
    <MainLayout title="Acceso restringido" subtitle="No tienes permiso para ver este reporte">
      <div className="flex items-center justify-center">
        <Card className="text-center w-full">
          ...
          <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-2">Acceso restringido</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">
            No tienes permiso para ver este reporte. Es posible que ya no tengas acceso al taladro asociado.
          </p>
          <Button onClick={() => navigate('/dashboard')} icon={<ArrowLeft size={16} />}>Volver al inicio</Button>
        </Card>
      </div>
    </MainLayout>
  );
    ...
    <MainLayout
      ...
      headerActions={
        <div className="flex gap-2 flex-wrap">
          <Button variant="outline" onClick={() => navigate(backPath)} icon={<ArrowLeft size={16} />}>Volver</Button>
      ...
      <div className="space-y-6 max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
          <div className="space-y-1">
            <div className="flex items-center gap-3 flex-wrap">
              <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-50">{report.wellNumber || 'Sin pozo'}</h2>
            ...
            <div className="flex items-center gap-4 text-sm text-gray-500 dark:text-gray-400 pt-1">
              <span className="inline-flex items-center gap-1.5"><Calendar size={14} />Fecha reporte: {formatDateDMY(report.reportDate)}</span>
              <span className="hidden sm:inline text-gray-300 dark:text-gray-600">|</span>
              <span className="hidden sm:inline">Creado: {formatDateTime(report.createdAt)}</span>
              {report.updatedAt && report.updatedAt !== report.createdAt && (
                <><span className="hidden sm:inline text-gray-300 dark:text-gray-600">|</span>
                <span className="hidden sm:inline">Editado: {formatDateTime(report.updatedAt)}</span></>
              )}
            </div>
          </div>
        ...
        <div className="flex items-center justify-between border-b border-gray-200 dark:border-gray-700">
          <div className="flex gap-1">
            {([{ key: 'report', label: 'Reporte' }, { key: 'review', label: 'Revisión' }] as const).map((tab) => (
          ...
          <div className="flex items-center gap-2 pb-1">
            {canEdit() && <Button variant="outline" size="sm" onClick={() => navigate(`/reports/edit/${id}`)} icon={<Edit size={14} />}>Editar</Button>}
            {canSubmit() && <Button variant="primary" size="sm" onClick={handleSubmit} disabled={actionLoading} loading={actionLoading} icon={<Send size={14} />}>Enviar</Button>}
            {canApprove() && (<>
              <Button variant="success" size="sm" onClick={handleApprove} icon={<CheckCircle size={14} />}>Aprobar</Button>
              <Button variant="danger" size="sm" onClick={handleReject} icon={<XCircle size={14} />}>Rechazar</Button>
            </>)}
          </div>
        </div>
        ...
        {viewTab === 'report' && (<>
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700 border border-gray-200 dark:border-gray-700 rounded">
                <thead className="bg-gray-50 dark:bg-gray-800"><tr>
                  <TH align="left">Código</TH><TH>Mañana (hrs)</TH><TH>Tarde (hrs)</TH><TH>Noche (hrs)</TH><TH>Total</TH>
                </tr></thead>
          ...
          {bitRecords.length > 0 && <Card><div className="p-6">
            ...
            <SectionCarousel slides={bitRecords.map((bit, idx) => ({
              ...
              content: <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                <DataField label="Tamaño" value={bit.size} /><DataField label="Marca" value={bit.brand} />
                <DataField label="Tipo" value={bit.bitType} /><DataField label="Serial" value={bit.serialNumber} />
                <DataField label="Jets" value={bit.jets} /><DataField label="TFA" value={bit.tfa} />
                <DataField label="Prof. Entrada" value={bit.depthIn} /><DataField label="Prof. Salida" value={bit.depthOut} />
                <DataField label="Metraje" value={bit.footage} /><DataField label="Horas Total" value={bit.hoursTotal?.toString()} />
              </div>,
            }))} />
          </div></Card>}
          ...
          {mudAdditives.length > 0 && <Card><div className="p-6">
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700 border border-gray-200 dark:border-gray-700 rounded">
                <thead className="bg-gray-50 dark:bg-gray-800"><tr>
                  <TH align="left">Turno</TH><TH align="left">Tipo</TH><TH align="left">Cantidad</TH>
                </tr></thead>
                <tbody className="bg-gray-50 dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
            ...
            <SectionCarousel slides={drillingParams.map((param, idx) => ({
              ...
              content: <div>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm mb-3">
                  <DataField label="RPM" value={param.rotaryRpm} /><DataField label="Peso Mecha" value={param.bitWeight} />
                  <DataField label="Presión" value={param.pumpPressure} /><DataField label="GPM" value={param.totalGpm} />
                  <DataField label="# Bomba" value={param.pumpNumber} /><DataField label="Liner Bomba" value={param.pumpLiner} />
                  <DataField label="SPM" value={param.pumpSpm} /><DataField label="Método" value={param.methodUsed} />
                </div>
          ...
          {deviationHistory.length > 0 && <Card><div className="p-6">
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700 border border-gray-200 dark:border-gray-700 rounded">
                <thead className="bg-gray-50 dark:bg-gray-800"><tr>
                  <TH align="left">Profundidad (ft)</TH><TH align="left">Desviación (°)</TH>
                  <TH align="left">Dirección</TH><TH align="left">TVO (ft)</TH><TH align="left">Desp. Horizontal (ft)</TH>
                </tr></thead>
                <tbody className="bg-gray-50 dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
            ...
            <SectionCarousel slides={operationsLog.map((op, idx) => ({
              ...
              content: <div className="space-y-2">
                <div className="flex items-center gap-3 text-sm text-gray-600 dark:text-gray-400">
                  ...
                  {op.duration && <span>Duración: {op.duration}</span>}
                </div>
          ...
          {drillStringComponents.length > 0 && <Card><div className="p-6">
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700 border border-gray-200 dark:border-gray-700 rounded">
                <thead className="bg-gray-50 dark:bg-gray-800"><tr>
                  <TH align="left">N°</TH><TH align="left">Pieza</TH><TH>Longitud (ft)</TH>
                </tr></thead>
                <tbody className="bg-gray-50 dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
...
function CrewShiftTable({ shift }: { shift: CrewShift }) {
  ...
  return <div className="overflow-x-auto">
    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700 border border-gray-200 dark:border-gray-700 rounded">
      <thead className="bg-gray-50 dark:bg-gray-800"><tr>
        <TH align="left">Posición</TH><TH align="left">CI</TH><TH align="left">Nombre</TH><TH align="left">Horas</TH>
      </tr></thead>
      <tbody className="bg-gray-50 dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
...
function groupMudRecordsByShift(records: MudRecord[]) {
  ...
  for (const [key, items] of grouped) {
    slides.push({ label: key === 'unknown' ? 'Sin turno' : key, content: <MudRecordsTable records={items} /> });
  }
  return slides;
}
  ...
  return <div className="overflow-x-auto">
    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700 border border-gray-200 dark:border-gray-700 rounded">
      <thead className="bg-gray-50 dark:bg-gray-800"><tr>
        ...
        <TH>Peso (ppg)</TH><TH>Visc. (seg)</TH><TH>PVP (cps)</TH>
        <TH>Geles</TH><TH>Filtrado</TH><TH>pH</TH><TH>Sólidos</TH>
      </tr></thead>
      <tbody className="bg-gray-50 dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
