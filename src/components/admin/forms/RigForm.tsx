...
export default function RigForm({ onSubmit, rig, areas, operators }: RigFormProps) {
  // If editing an existing rig, skip wizard and show unified form
  const isEditing = !!rig?.id;

  // For new rigs: wizard state
  const [step, setStep] = useState<1 | 2>(1);
  const [savedRigId, setSavedRigId] = useState<string | null>(null);
  const [savedRigName, setSavedRigName] = useState<string>('');

  const handleRigCreated = (rigId: string, rigName: string) => {
    setSavedRigId(rigId);
    setSavedRigName(rigName);
    setStep(2);
  };

  // If editing, show the old unified form
  if (isEditing) {
    return (
      <EditRigForm
        rig={rig}
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-3 pb-4 mb-4 border-b border-gray-200 dark:border-gray-700">
        ...
        <div className="flex-1 min-w-0">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100">
            {step === 1 ? 'Datos del Taladro' : 'Personal de Cuadrilla'}
          </h3>
          <p className="text-sm text-gray-500 dark:text-gray-400">
            {step === 1
              ? 'Ingresa la información básica del taladro'
              : `Agrega el personal para ${savedRigName}`}
          </p>
        </div>
      ...
      {step === 1 ? (
        <StepRigData
          ...
          operators={operators}
          onSubmit={onSubmit}
          onRigCreated={handleRigCreated}
        />
      ) : savedRigId ? (
        <StepPersonnel
          rigId={savedRigId}
          rigName={savedRigName}
          onBack={() => setStep(1)}
        />
      ) : null}
    </div>
...
// ============================================================================
// Step 1 — Rig Data (Create)
// ============================================================================

interface StepRigDataProps {
  areas: Area[];
  operators: Operator[];
  onSubmit: (data: CreateRigInput) => Promise<string | void>;
  onRigCreated: (rigId: string, rigName: string) => void;
}

function StepRigData({ areas, operators, onSubmit, onRigCreated }: StepRigDataProps) {
  const {
    register,
    handleSubmit,
...
// ============================================================================
// Step 2 — Personnel Management (After Rig Created)
// ============================================================================
...
function StepPersonnel({ rigId, rigName, onBack }: StepPersonnelProps) {
  ...
  return (
    <div className="flex flex-col flex-1">
      {/* Badge with rig name */}
      <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm font-medium mb-4 self-start bg-primary-50 dark:bg-primary-900/20 text-primary-700 dark:text-primary-400">
        <Settings size={14} />
        {rigName}
      </div>
      ...
      <div className="flex justify-between gap-3 pt-4 mt-4 border-t border-gray-200 dark:border-gray-700">
        <Button
          ...
          disabled
          title="El taladro ya fue creado"
        >
          Atrás
        </Button>
        <Button
...
// ============================================================================
// Edit Form (For Existing Rigs - Single Page)
// ============================================================================
...
function EditRigForm({ rig, areas, operators, onSubmit }: EditRigFormProps) {
  const {
    register,
    handleSubmit,
    formState: { errors, isSubmitting },
...
// ============================================================================
// Personnel Management Section (For Edit Mode)
// ============================================================================
