...
export async function exportReportToPDF(data: ReportData): Promise<void> {
  ...
  doc.text(`Reporte #${report.reportNumber} | ${new Date(report.reportDate).toLocaleDateString()}`, pageWidth / 2, 25, { align: 'center' });
  ...
  if (data.crewShifts && data.crewShifts.length > 0) {
    ...
    data.crewShifts.forEach(shift => {
      ...
      doc.setFontSize(11);
      doc.setFont('helvetica', 'bold');
      doc.text(`${SHIFT_LABELS[shift.shift]} (${shift.shiftStart} - ${shift.shiftEnd})`, margin, yPos);
      yPos += 7;
  ...
  if (data.timeDistributions && data.timeDistributions.length > 0) {
    ...
    const timeData = data.timeDistributions.map(td => [
      ...
      td.hoursShift1.toString(),
      td.hoursShift2.toString(),
      td.hoursShift3.toString(),
      (td.hoursShift1 + td.hoursShift2 + td.hoursShift3).toString() + 'h'
    ]);
  ...
  if (data.mudRecords && data.mudRecords.length > 0) {
    ...
    const mudData = data.mudRecords.map(mud => [
      mud.shift ? SHIFT_LABELS[mud.shift] : '-',
      mud.hour || '-',
      mud.weight || '-',
      mud.viscosity || '-',
      mud.pvp || '-',
  ...
  // Save PDF
  const filename = `DDR_${report.reportNumber}_${report.reportDate}.pdf`;
  doc.save(filename);
}
